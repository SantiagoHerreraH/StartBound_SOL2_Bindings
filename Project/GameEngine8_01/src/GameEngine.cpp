...
RECT GameEngine::GetStringBounds(const tstring& text, int left, int top) const
{
	HFONT old = nullptr;
	if (m_FontDraw) old = (HFONT)SelectObject(m_HdcDraw, m_FontDraw);

	SIZE sz{};                       // width/height of the glyph run
	GetTextExtentPoint32(m_HdcDraw, text.c_str(), (int)text.size(), &sz);

	TEXTMETRIC tm{};
	GetTextMetrics(m_HdcDraw, &tm);

	UINT ta = GetTextAlign(m_HdcDraw);     // respect TA_LEFT/CENTER/RIGHT and TOP/BOTTOM/BASELINE
	int x = left;
	if ((ta & TA_CENTER) == TA_CENTER)      x -= sz.cx / 2;
	else if ((ta & TA_RIGHT) == TA_RIGHT)   x -= sz.cx;  // TA_LEFT is default

	int yTop;
	if ((ta & TA_BASELINE) == TA_BASELINE)  yTop = top - tm.tmAscent;
	else if ((ta & TA_BOTTOM) == TA_BOTTOM) yTop = top - sz.cy;
	else                                    yTop = top;  // TA_TOP (default)

	if (old) SelectObject(m_HdcDraw, old);
	return RECT{ x, yTop, x + sz.cx, yTop + sz.cy };
}

int GameEngine::DrawString(const tstring& text, int left, int top, RECT* outRect) const
{
	if (!m_IsPainting) return -1;
	...
	// optional: return the bounding rect of what will be drawn
	if (outRect) {
		outRect->left = left;
		outRect->top = top;
		outRect->right = left + sz.cx;
		outRect->bottom = top + sz.cy;
	}
...
{
	/*Gdiplus::Bitmap* bitmapPtr = Gdiplus::Bitmap::FromFile(reinterpret_cast<const WCHAR*>(pFilePath), FALSE);
	...
	if (!pFilePath) return nullptr;

	// Make a real wide string for GDI+
	std::wstring wpath;
#ifdef UNICODE
	wpath = pFilePath; // already wide
#else
	// Convert ANSI (or your ACP) to UTF-16
	int n = MultiByteToWideChar(CP_ACP, 0, pFilePath, -1, nullptr, 0);
	if (n <= 1) return nullptr;
	wpath.resize(n - 1);
	MultiByteToWideChar(CP_ACP, 0, pFilePath, -1, wpath.data(), n);
#endif

	std::unique_ptr<Gdiplus::Bitmap> bmp(Gdiplus::Bitmap::FromFile(wpath.c_str(), FALSE));
	if (!bmp || bmp->GetLastStatus() != Gdiplus::Ok) return nullptr;

	HBITMAP hbm = nullptr;
	if (bmp->GetHBITMAP(Gdiplus::Color(0, 0, 0), &hbm) != Gdiplus::Ok) return nullptr;
	return hbm;
}
