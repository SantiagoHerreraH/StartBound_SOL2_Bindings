...
public:
	...
	bool		SetWindowRegion		(const HitRegion* regionPtr); //done
	...
	template	<typename MyOtherType> //done
	void		MessageBox			(MyOtherType value)						const	{ MessageBox(to_tstring(value)); }

	bool		MessageContinue		(const tstring& message)				const; //done

	// Text Dimensions
	SIZE		CalculateTextDimensions(const tstring& text, const Font* fontPtr)							const; //done
	SIZE		CalculateTextDimensions(const tstring& text, const Font* fontPtr, RECT rect)				const; //done

	// Draw Functions
	void		SetColor(BYTE r, BYTE g, BYTE b) { SetColor(RGB(r, g, b)); }; //done
	...
	bool		DrawOval			(int left, int top, int right, int bottom)								const; //done
	bool		FillOval			(int left, int top, int right, int bottom)								const; //done
	bool		FillOval			(int left, int top, int right, int bottom, int opacity)					const; //done
	bool		DrawArc				(int left, int top, int right, int bottom, int startDegree, int angle)	const; //done
	bool		FillArc				(int left, int top, int right, int bottom, int startDegree, int angle)	const; //done

	RECT		GetStringBounds		(const tstring& text, int left, int top)const;
	int			DrawString			(const tstring& text, int left, int top, RECT* outRect)					const;
	...
	bool		DrawBitmap			(const Bitmap* bitmapPtr, int left, int top)							const; //done
	bool		DrawBitmap			(const Bitmap* bitmapPtr, int left, int top, RECT sourceRect)			const; //done
	...
	COLORREF	GetDrawColor		()						const; //done
	bool		Repaint				()						const; //done

	// Accessor Member Functions	
	tstring		GetTitle			()						const; //done
	HINSTANCE	GetInstance			()						const	{ return m_Instance; } //done
	HWND		GetWindow			()						const	{ return m_Window; } //done
	...
	POINT		GetWindowPosition	()						const; //done

	// Tab control
	void		TabNext				(HWND ChildWindow)		const; //done
 	void		TabPrevious			(HWND ChildWindow)		const; //done
		
private:
	// Private Member Functions	//non accessible by sol2
...
class Timer : public Caller //done, added /bigobj to facilitate it
{
public:
	// -------------------------
...
class TextBox : public Caller //done
{
public:
	// -------------------------
	// Constructor(s) and Destructor
